...
namespace BotOperate.Services
{
    public sealed class BotService : IBotService
    {
        ...
        public BotService(ITicketService positionService,
        ...
        public async Task<bool> HandleAdaptiveCardAction(Activity activity, CancellationToken cancellationToken)
        {
            ...
            switch (command.CommandId)
            {
                case AppCommands.OpenNewTicket:
                    result = await OpenNewTicket(activity, cancellationToken);                    
                    break;
            ...
            //client.Conversations.ReplyToActivity((Activity)message);
        ...
        private async Task<AdaptiveCard> OpenNewTicket(Activity activity, CancellationToken cancellationToken)
        {
            var commandData = JsonConvert.DeserializeObject<TicketCreateCommand>(activity.Value?.ToString());
            if (commandData is null)
            {
                return null;
            }                    
            var position = await _positionService.AddNewTicket(commandData, cancellationToken);
